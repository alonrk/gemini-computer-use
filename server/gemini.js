...
function buildSystemInstruction(session) {
  return [
    ...
    "After each action, verify state change using URL/title/content and tool result, but do not over-verify when the next relevant target is already clearly visible.",
    ...
    "When the page already shows a clear next-step control that matches the user goal, prefer acting on it instead of waiting for extra confirmation.",
    "Do not use wait_5_seconds as a substitute for an obvious next action on the current page.",
